...
부화 → 진화 → 졸업, 도감(종별 묶음+진화 라인 보기, 1세대 151/151 채우면 2세대
해금), 보관함(성장 중 교체), 알 보관함(등급별 부화 + 매주 무료 티켓), 이로치,
애니메이션 스프라이트, 항상 떠있는 위젯까지 Windows 실기에서 직접 확인 완료.
Windows 알림(부화/진화/졸업/알 티켓/2세대 해금 시)도 코드는 붙였는데, 실제
토스트가 뜨는지는 아직 Windows 실기 확인 전.
...
이전에 먼저 해결해서 151/151이 구조적으로 달성 가능해져 있었고, 이번에 실제
2세대 데이터 + 해금 로직을 붙였다.
...
`generation: 1|2` 필드로 구분.

**해금 조건(사용자 확정, 구현 완료)**: `state.unlockedGen`(기본 1) — 매 tick마다
`checkGenUnlock()`이 1세대 발견 종 수를 세서 151에 도달하면 2로 승격. 부화 풀
(`pickHatchSpecies`)은 `maxGen` 인자로 필터링해서 안 열린 세대 종은 애초에 후보에
안 섞임 — 도감도 "발견 기반"이라 원천적으로 2세대가 안 보임(별도 숨김 로직 불필요).
도감 분모("N/151" ↔ "N/251")는 `pokedexTotal`로 서버에서 계산해서 내려줌 — 해금
전후로 자동 전환됨. 별도의 "🔒 2세대 잠김" 안내 화면은 안 만듦(분모가 커지는 것
자체로 해금을 알 수 있어서) — 필요하면 나중에 추가.
