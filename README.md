...
Claude Code 토큰 사용량으로 포켓몬(1~4세대, 493마리)을 키우는 개인용 Windows
트레이 앱 MVP.

## 현재 상태: 동작 확인됨

부화 → 진화 → 졸업, 도감(493마리 전체 표시·미발견은 실루엣, 1~4세대 전부
...
```bash
npm install
npm run build-data     # PokéAPI에서 1~4세대 493마리 데이터 fetch (인터넷 필요, 1회만, 좀 걸림)
...
없음) — 이제 1~4세대 다 담겨있다는 점만 로딩 부분 주석으로 남겨둠.
...
- **메가진화/기가맥스/테라스탈 같은 "부스트" 기믹 (이름 미정)** — 레퍼런스
  ...
  **어떤 종이 어떤 기믹을 쓸 수 있는지 — PokéAPI로 실측 확인(1~493,
  1~4세대 기준. 4세대 추가 후 재확인 — 메가진화/기가맥스 수는 그대로였음,
