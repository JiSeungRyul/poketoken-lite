...
Claude Code 토큰 사용량으로 포켓몬(1~3세대, 386마리)을 키우는 개인용 Windows
트레이 앱 MVP.

## 현재 상태: 동작 확인됨

부화 → 진화 → 졸업, 도감(386마리 전체 표시·미발견은 실루엣, 1~3세대 전부
...
```bash
npm install
npm run build-data     # PokéAPI에서 1~3세대 386마리 데이터 fetch (인터넷 필요, 1회만, 좀 걸림)
...
- **4~9세대 추가** — 2·3세대 때 이미 세대 수 무관하게 일반화해둔 구조라
  `scripts/build-pokedex-data.js`의 `GENERATIONS` 배열에 한 줄씩만 추가하고
  재빌드하면 되는 기계적인 작업(게임 로직은 안 건드림, 3세대 추가 때 검증된
  패턴 그대로). PokéAPI로 실측 확인한 정확한 범위(전부 빈틈없이 연속):
  - 4세대(신오): `{ gen: 4, min: 387, max: 493 }` (107마리)
  - 5세대(하나): `{ gen: 5, min: 494, max: 649 }` (156마리)
  - 6세대(칼로스): `{ gen: 6, min: 650, max: 721 }` (72마리)
  - 7세대(알로라): `{ gen: 7, min: 722, max: 809 }` (88마리)
  - 8세대(가라르): `{ gen: 8, min: 810, max: 905 }` (96마리)
  - 9세대(팔데아): `{ gen: 9, min: 906, max: 1025 }` (120마리)
  - 다 더하면 1025마리. 한 번에 다 넣기보다 세대별로 나눠서 재빌드(빌드 시간
    분산 + 크로스세대 진화 클리핑 세대마다 스팟체크하기 쉬움) 추천.
