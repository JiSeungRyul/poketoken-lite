...
Claude Code 토큰 사용량으로 포켓몬(1~8세대, 905마리)을 키우는 개인용 Windows
트레이 앱 MVP.

## 현재 상태: 동작 확인됨

부화 → 진화 → 졸업, 도감(905마리 전체 표시·미발견은 실루엣, 1~8세대 전부
...
```bash
npm install
npm run build-data     # PokéAPI에서 1~8세대 905마리 데이터 fetch (인터넷 필요, 1회만, 좀 걸림)
...
없음) — 이제 1~8세대 다 담겨있다는 점만 로딩 부분 주석으로 남겨둠.

**5세대(하나, 494~649)·6세대(칼로스, 650~721)·7세대(알로라, 722~809)·
8세대(가라르, 810~905) 추가**: 전부 배열에 한 줄씩 추가+재빌드, 전체
stage/maxStage/evolvesTo 일관성 스윕(4세대 때 만든 "샌드위치 체인" 체크
포함) 통과, 특이사항 없음. 8세대 추가로 갈라르 네이티브 기가맥스 18종이
새로 생겨서 "부스트" 기믹 백로그의 기가맥스 목록을 12종→30종으로 갱신함
(레쿠쟈 등 메가진화 39종은 그대로 — 공식 메가진화는 6세대까지만 존재).
...
- **메가진화/기가맥스/테라스탈 같은 "부스트" 기믹 (이름 미정)** — 레퍼런스
  ...
  **어떤 종이 어떤 기믹을 쓸 수 있는지 — PokéAPI로 실측 확인(1~905,
  1~8세대 기준. 8세대 추가로 기가맥스 목록이 처음 늘어남 — 아래 참고).**
  ...
  - **테라스탈**: 종 제한 없음 — 지금 있는 905종 전부(나중에 더 늘어나도
    마찬가지) 적용 가능.
  ...
  **보상 종류 — 사용자 제안(사탕/알/상점 재화) + 추가 아이디어(전부 미정,
  ...
  4. 추가로 고민할 지점(아직 미정): 부스팅 껐을 때 게이지 진행분이 유지되는지
     ...
     반복 사용 가능 여부·쿨다운 유무.
- **9세대 추가** — 2~8세대 때 이미 세대 수 무관하게 일반화해둔 구조라
  `scripts/build-pokedex-data.js`의 `GENERATIONS` 배열에 한 줄만 추가하고
  재빌드하면 되는 기계적인 작업(게임 로직은 안 건드림, 4세대 추가 때 발견한
  "샌드위치 체인" 버그도 이미 고쳐져 있어서 그 패턴 걱정 없이 진행 가능).
  PokéAPI로 실측 확인한 정확한 범위: 9세대(팔데아) `{ gen: 9, min: 906,
  max: 1025 }` (120마리) — 다 더하면 1025마리(이게 마지막 세대).
