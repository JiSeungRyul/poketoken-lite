...
Claude Code 토큰 사용량으로 포켓몬(1~7세대, 809마리)을 키우는 개인용 Windows
트레이 앱 MVP.

## 현재 상태: 동작 확인됨

부화 → 진화 → 졸업, 도감(809마리 전체 표시·미발견은 실루엣, 1~7세대 전부
...
```bash
npm install
npm run build-data     # PokéAPI에서 1~7세대 809마리 데이터 fetch (인터넷 필요, 1회만, 좀 걸림)
...
없음) — 이제 1~7세대 다 담겨있다는 점만 로딩 부분 주석으로 남겨둠.

**5세대(하나, 494~649)·6세대(칼로스, 650~721)·7세대(알로라, 722~809) 추가**:
전부 배열에 한 줄씩 추가+재빌드, 전체 stage/maxStage/evolvesTo 일관성
스윕(4세대 때 만든 "샌드위치 체인" 체크 포함) 통과, 특이사항 없음.
...
- **메가진화/기가맥스/테라스탈 같은 "부스트" 기믹 (이름 미정)** — 레퍼런스
  ...
  **어떤 종이 어떤 기믹을 쓸 수 있는지 — PokéAPI로 실측 확인(1~809,
  1~7세대 기준. 4~7세대 추가 후 재확인 — 메가진화/기가맥스 수는 그대로였음,
  ...
  - **기가맥스 가능 12종** (지금 데이터 안에 있는 것만 — 갈라르 도감 기준
    ...
    잠만보.
  - **테라스탈**: 종 제한 없음 — 지금 있는 809종 전부(나중에 더 늘어나도
    마찬가지) 적용 가능.
  ...
  **보상 종류 — 사용자 제안(사탕/알/상점 재화) + 추가 아이디어(전부 미정,
  ...
  4. 추가로 고민할 지점(아직 미정): 부스팅 껐을 때 게이지 진행분이 유지되는지
     ...
     반복 사용 가능 여부·쿨다운 유무.
- **8~9세대 추가** — 2~7세대 때 이미 세대 수 무관하게 일반화해둔 구조라
