...
- **메가진화/테라스탈/다이맥스 같은 "부스트" 기믹 (이름 미정)** — 레퍼런스
  (PokeTokenBar)엔 없는 기믹, 본가 게임 컨셉에서 따온 사용자 아이디어라
  검증된 설계는 아니고 구상 단계 메모. 사용자가 정리한 규칙:
  ...
  - 부스트 게이지를 다 채우면 보상 지급 — **보상 종류(사탕/알/상점 재화 중
    뭘 줄지)는 아직 미정**, 나중에 정할 것.
  - 추가로 고민할 지점(아직 미정): 부스팅 껐을 때 게이지 진행분이 유지되는지
