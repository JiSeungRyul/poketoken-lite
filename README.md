...
없음) — 이제 1~9세대(전체) 다 담겨있다는 점만 로딩 부분 주석으로 남겨둠.

**5세대(하나, 494~649)·6세대(칼로스, 650~721)·7세대(알로라, 722~809)·
8세대(가라르, 810~905)·9세대(팔데아, 906~1025) 추가**: 전부 배열에 한
줄씩 추가+재빌드, 전체 stage/maxStage/evolvesTo 일관성 스윕(4세대 때 만든
"샌드위치 체인" 체크 포함) 통과, 특이사항 없음. 8세대 추가로 갈라르 네이티브
기가맥스 18종이 새로 생겨서 "부스트" 기믹 백로그의 기가맥스 목록을
12종→30종으로 갱신함. 9세대는 메가진화/기가맥스 둘 다 추가 없음 —
Scarlet/Violet은 다이맥스/기가맥스 자체가 없는 세대(테라스탈만 있음)라
예상대로였고, PokéAPI에 9세대 종 몇 개에 "mega" variety가 붙어있던 건
확인해보니 전부 `mega-dimension`(팬메이드, 4세대 항목에서도 걸러냈던 것과
같은 비공식 버전그룹)이라 제외.

**1~9세대(전 세대) 완료 — 총 1025마리.** 여기서부터는 새 세대가 나올 때까지
추가할 게 없음.
...
- **메가진화/기가맥스/테라스탈 같은 "부스트" 기믹 (이름 미정)** — 레퍼런스
  ...
  **어떤 종이 어떤 기믹을 쓸 수 있는지 — PokéAPI로 실측 확인(1~1025,
  1~9세대 전 세대 기준 — 이제 더 늘어날 일 없음, 다음 세대가 나오기 전까진).**
  메가진화/기가맥스는 본가에서도 정해진 종만 가능한
  ...
  **보상 방식 — 대화로 합의된 설계(확정, 구현만 남음):**
  ...
  섹션 참고), 이 요구사항이 자동으로 충족된다. 새 아이템 종류·새 데이터 구조
  전혀 안 만들어도 됨 — 상점에서 산 사탕이랑 완전히 같은 풀에 섞여 들어감.
